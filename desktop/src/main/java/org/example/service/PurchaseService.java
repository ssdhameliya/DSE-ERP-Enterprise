...
public class PurchaseService {
    ...
    public List<Purchase> allFiltered(String q,String supplier,LocalDate from,LocalDate to,String paymentStatus,String due,String mail,String documentStatus,String returnStatus){
        ...
        OperationsApiClient.PurchasePage first=api.purchasesPage(0,200,q,supplier,from,to,paymentStatus,due,mail,documentStatus,returnStatus);applyReturnSettlements(first.rows());List<Purchase> out=new ArrayList<>(first.rows()==null?List.of():first.rows());
        for(int p=1;p<first.totalPages();p++){OperationsApiClient.PurchasePage next=api.purchasesPage(p,200,q,supplier,from,to,paymentStatus,due,mail,documentStatus,returnStatus);applyReturnSettlements(next.rows());if(next.rows()!=null)out.addAll(next.rows());}
        return out;
    }
