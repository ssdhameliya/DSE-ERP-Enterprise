...
public class SalesService {
    ...
    public OperationsApiClient.SalesPage page(int page,int size,String q,String invoice,String customer,LocalDate from,LocalDate to,String paymentStatus,String due,String mail,String whatsapp,String invoiceType,String documentStatus,String returnStatus,Double minAmount,Double maxAmount){return page(page,size,q,invoice,customer,from,to,paymentStatus,due,mail,whatsapp,invoiceType,documentStatus,returnStatus,minAmount,maxAmount,true);}
    public OperationsApiClient.SalesPage page(int page,int size,String q,String invoice,String customer,LocalDate from,LocalDate to,String paymentStatus,String due,String mail,String whatsapp,String invoiceType,String documentStatus,String returnStatus,Double minAmount,Double maxAmount,boolean includeSummary){if(useApi()){OperationsApiClient.SalesPage result=api.salesPage(page,size,q,invoice,customer,from,to,paymentStatus,due,mail,whatsapp,invoiceType,documentStatus,returnStatus,minAmount,maxAmount,includeSummary,includeSummary&&!PlatformUiSupport.isMac(),false);applyReturnSettlements(result.rows());return result;}List<Sales> source=new ArrayList<>(dao.getAll());List<Sales> filtered=source.stream().filter(x->match(x,q,invoice,customer,from,to,paymentStatus,due,mail,whatsapp,invoiceType,documentStatus,minAmount,maxAmount)&&matchReturnStatus(x,returnStatus)).toList();int safeSize=Math.max(10,Math.min(size,200)),pages=filtered.isEmpty()?0:(int)Math.ceil(filtered.size()/(double)safeSize),safePage=pages==0?0:Math.min(Math.max(0,page),pages-1),start=Math.min(safePage*safeSize,filtered.size()),end=Math.min(start+safeSize,filtered.size());List<Sales> activeFiltered=filtered.stream().filter(this::active).toList();var totals=includeSummary?new OperationsApiClient.RegisterTotals(activeFiltered.size(),sum(activeFiltered,Sales::getTotalAmount),sum(activeFiltered,Sales::getPaidAmount),sum(activeFiltered,Sales::getBalanceAmount)):null;return new OperationsApiClient.SalesPage(List.copyOf(filtered.subList(start,end)),safePage,safeSize,filtered.size(),pages,totals,includeSummary?localMetrics(filtered):null,List.of());}
    ...
    public List<Sales> allFiltered(String q,String invoice,String customer,LocalDate from,LocalDate to,String paymentStatus,String due,String mail,String whatsapp,String invoiceType,String documentStatus,String returnStatus,Double minAmount,Double maxAmount){
        ...
        OperationsApiClient.SalesPage first=api.salesPage(0,200,q,invoice,customer,from,to,paymentStatus,due,mail,whatsapp,invoiceType,documentStatus,returnStatus,minAmount,maxAmount,false,false,false);applyReturnSettlements(first.rows());List<Sales> out=new ArrayList<>(first.rows()==null?List.of():first.rows());
        for(int p=1;p<first.totalPages();p++){OperationsApiClient.SalesPage next=api.salesPage(p,200,q,invoice,customer,from,to,paymentStatus,due,mail,whatsapp,invoiceType,documentStatus,returnStatus,minAmount,maxAmount,false,false,false);applyReturnSettlements(next.rows());if(next.rows()!=null)out.addAll(next.rows());}
        return out;
    }
