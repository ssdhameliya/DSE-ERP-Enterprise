...
public final class OperationsApiClient {
    ...
    public SalesPage salesPage(int page,int size,String q,String invoice,String customer,LocalDate from,LocalDate to,String paymentStatus,String due,String mail,String whatsapp,String invoiceType,String documentStatus,String returnStatus,Double minAmount,Double maxAmount){
        String path="/api/operations/sales/page?page="+Math.max(0,page)+"&size="+Math.max(10,Math.min(size,200))+"&q="+enc(q)+"&invoice="+enc(invoice)+"&customer="+enc(customer)+"&from="+enc(str(from))+"&to="+enc(str(to))+"&paymentStatus="+enc(paymentStatus)+"&due="+enc(due)+"&mail="+enc(mail)+"&whatsapp="+enc(whatsapp)+"&invoiceType="+enc(invoiceType)+"&documentStatus="+enc(documentStatus)+"&returnStatus="+enc(returnStatus);
    ...
    public PurchasePage purchasesPage(int page,int size,String q,String supplier,LocalDate from,LocalDate to,String paymentStatus,String due,String mail,String documentStatus,String returnStatus){PurchasePageDto d=get("/api/operations/purchases/page?page="+Math.max(0,page)+"&size="+Math.max(10,Math.min(size,200))+"&q="+enc(q)+"&supplier="+enc(supplier)+"&from="+enc(str(from))+"&to="+enc(str(to))+"&paymentStatus="+enc(paymentStatus)+"&due="+enc(due)+"&mail="+enc(mail)+"&documentStatus="+enc(documentStatus)+"&returnStatus="+enc(returnStatus),PurchasePageDto.class);return new PurchasePage(d.rows()==null?List.of():d.rows().stream().map(this::purchase).toList(),d.page(),d.size(),d.totalRows(),d.totalPages(),d.filteredTotals(),d.metrics(),d.suppliers()==null?List.of():d.suppliers());}
