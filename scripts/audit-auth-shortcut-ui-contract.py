...
multi_user = text('scripts/Enable Multi-User.ps1')
...
require('DeploymentConnectionService.inspect(normalizeBaseUrl(serverBaseUrl))' in auth and 'BuildInfo.version().equals(status.version())' not in auth and 'BuildInfo.buildRevision().equals(status.buildRevision())' not in auth and 'org.example.update.BuildInfo.version().equals(status.version())' not in auth and 'org.example.update.BuildInfo.buildRevision().equals(status.buildRevision())' not in auth, 'Auth client must delegate login/MFA/session compatibility to the centralized Shared Client policy without exact remote version equality')
require('minimumSupportedDesktopVersion' in multi_user and '$health.version -ne $ExpectedVersion' not in multi_user and '$desktopVersion -lt $minimumVersion' in multi_user and '$desktopVersion -gt $serverVersion' in multi_user, 'Enable Multi-User must validate the compatibility window instead of requiring an exact remote server release')
