...
need('EXPECTED_MINIMUM_DESKTOP' in oracle and "r.get('minimumSupportedDesktopVersion') == minimum" in oracle,'Oracle deployment does not enforce the release-owned desktop compatibility floor')
release=t('.github/workflows/release.yml'); prod=t('.github/workflows/deploy-prod.yml')
need('compatibility_baseline' in release and 'minimumSupportedDesktopVersion' in release,'UAT release workflow does not propagate/verify the compatibility baseline')
need('COMPATIBILITY_BASELINE' in prod and 'minimumSupportedDesktopVersion' in prod,'PROD workflow does not propagate/verify the compatibility baseline')
