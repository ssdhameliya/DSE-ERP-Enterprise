...
icons = (JAVA / 'org/example/util/IconFactory.java').read_text(encoding='utf-8')
...
for p in FXML.rglob('*.fxml'):
    for m in re.finditer('<TableColumn\\b[^>]*\\b(?:prefWidth|minWidth|maxWidth)\\s*=', p.read_text(encoding='utf-8'), re.I):
...
enhancer = (JAVA / 'org/example/util/ProfessionalUiEnhancer.java').read_text(encoding='utf-8')
...
kpi = (JAVA / 'org/example/util/ResponsiveKpiLayoutManager.java').read_text(encoding='utf-8')
...
coordinator = (JAVA / 'org/example/util/UiViewportLayoutCoordinator.java').read_text(encoding='utf-8')
register = (JAVA / 'org/example/util/RegisterUiSupport.java').read_text(encoding='utf-8')
