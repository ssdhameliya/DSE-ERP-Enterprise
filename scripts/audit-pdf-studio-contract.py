...
req('STRICT_FIXED' in builtin_installer and 'demoteExistingSalesDefaults' in builtin_installer and ('totals.amountInWordsText' in builtin_installer) and ('PdfStudioRemoteStore.publish' in builtin_installer), 'approved built-in Sales template must install once as fixed default and remain server-shareable')
...
req(hashlib.sha256(source_pdf.read_bytes()).hexdigest() == '08cfb11fb104aef7c7a84c1171a5a10886b8221f4ccdadb7dcebf6dfef3340f9', 'built-in Sales template PDF must remain byte-identical to the user-approved source PDF')
req('Available ERP / JSON Fields' in fxml and 'Browse / Drag ERP Fields' in fxml and ('#validateMapping' in fxml) and ('#publishAndSetDefault' in fxml) and ('#viewJsonData' in fxml) and ('Fixed PDF artwork' in fxml), 'PDF Studio UI must expose the simplified fixed-PDF JSON mapping workflow')
...
req('private static final int RELEASE_VERSION = 5;' in builtin_installer and '58.6688, 224.05' in builtin_installer and ('337.6388, 224.05' in builtin_installer), '9.0.79 built-in Sales template must migrate Billing/Delivery GSTIN to the corrected source-PDF baseline')
