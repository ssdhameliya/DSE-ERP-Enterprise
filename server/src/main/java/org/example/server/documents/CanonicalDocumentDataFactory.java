...
public class CanonicalDocumentDataFactory {
    ...
    private Map<String,ItemEntity> masters(List<OperationDtos.LineDto> lines){
        Set<String> codes=new LinkedHashSet<>();
        for(var line:lines==null?List.<OperationDtos.LineDto>of():lines)if(line!=null&&!safe(line.itemCode()).isBlank())codes.add(safe(line.itemCode()));
        if(codes.isEmpty())return Map.of();
        Map<String,ItemEntity> out=new HashMap<>();
        for(ItemEntity i:items.findByItemCodeIn(codes))if(i!=null)out.put(norm(i.getItemCode()),i);
        return out;
    }
